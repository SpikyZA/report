...
module.exports = {
    ...
    async execute(client, message, args, data) {
      ...
      if(args[0] == "prefix") {
        ...
        .setDescription("The ``prefix`` command is used to change the prefix for the bot in your server, only")
        .addField("Aliases", `p`)
        ...
        .setDescription("The ``reportchannel`` command is used to change the channel that the report logs goto")
        .addField("Aliases", `rc`)
        .addField("Permissions", `MANAGE_MESSAGES`)
        ...
        .setDescription("The ``serverinfo`` command send some useful information about your server, including member count, server id and others")
        .addField("Aliases", `se`)
        ...
        .setDescription("The ``botinfo`` command provides information about Report, including server count and other useless stats")
        .addField("Aliases", `bi`)
        .addField("Permissions", `NONE`)
      ...
      } else if(args[0] == "accept-deny") {
        let helpEmbed = new Discord.MessageEmbed()
        .setTitle("**ADMIN** How to Approve or Deny a suggestion")
        .setDescription("To approve or deny a suggestion, your staff will need the \`MANAGE_MESSAGES\` permission.\n\nTo **Approve**, you can do *r!suggest approve [suggestID]*\nReplace the *[suggestID]* with the Suggestion ID in the footer of the message. This will also send the suggestion to a Suggestion Approved channel (requires setup)\n\nTo **Deny** a suggestion, use *r!suggest deny [suggestID]*\nReplace *[suggestID]* with the suggestion ID in the footer of the suggestion")
        .setColor("#FA8334")
        message.channel.send(helpEmbed);
      }else {
        let helpEmbed = new Discord.MessageEmbed()
        .setTitle("Help Page")
        .setDescription("Welcome to the help embed for **Report**\n\nTo find out more information about a certain command, you can do *r!help [command]*\n\nIf you are having problems with setup, you can do ``r!setuphelp`` or join the support Discord")
        ...
        .addField("Suggest Commands", `\`suggest\` \`suggestchannel\` \`suggestapproval\` \`accept-deny\``)
